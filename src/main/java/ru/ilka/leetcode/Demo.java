...
public class Demo {
    ...
    // .orElseGet() does not execute false code.
    public static Optional<String> testOrElseGet(boolean present){
        return present ? Optional.of("present") : Optional.empty();
    }

    public static void staticCounterProblem(){
        Counter a = new Counter(100);
        ...
        System.out.println("a.getCounter() = " + a.getCounter());
        System.out.println("b.getCounter() = " + b.getCounter());
        System.out.println("c.getCounter() = " + c.getCounter());
    }

    public static boolean containsDuplicateElementsInWindow(Integer[] array, int windowSize) {
        if (windowSize >= array.length) {
            Set<Integer> set = new HashSet<>(Arrays.asList(array));
            return set.size() != array.length;
        } else {
            Map<Integer, Integer> windowElementsMap = new HashMap<>();
            for (int i = 0; i < array.length; i++) {
                if (i >= windowSize) {
                    windowElementsMap.remove(i - windowSize);
                }
                if (windowElementsMap.containsValue(array[i])) {
                    return true;
                }
                windowElementsMap.put(i, array[i]);
            }
            return false;
        }
    }

    public static int generateValue(int min, int max, List<Integer> excludedValues) {
        val allPriorities = IntStream.rangeClosed(min, max)
                .boxed()
                .collect(Collectors.toList());
        allPriorities.removeAll(excludedValues);
        Collections.shuffle(allPriorities);
        return allPriorities.stream()
                .findAny()
                .orElseThrow();
    }

    public static int generateValue2(int min, int max, List<Integer> excludedValues) {
        Set<Integer> excluded = new HashSet<>(excludedValues);
